...
配套客户端探测无响应或激活后长时间没有回复时，本流回到可靠流，但不放弃 raw：客户端发来的零长度帧只表示暂停下行 raw，绑定与 CID 认领保留，它下一个 raw 包即恢复双向 raw。raw 读写失败才是本流永久关闭。绑定 60 秒没有收到 raw 即停止承载下行，并允许同一客户端换端口重新绑定，使空闲期被 NAT 重新映射的流仍能恢复。可靠流关闭时清理目标连接与绑定。具体计时、CID 条件和帧格式见专题。
